...
int fs_write_file(const char *name, const char *content, uint32_t size) {
    uart_print("FS_DEBUG: Incercare scriere in: "); uart_print(name); uart_print("\n");
    uart_print("FS_DEBUG: Dimensiune primita: "); uart_print_int((int)size); uart_print("\n");
    ...
    while (curr) {
        if (curr->type == FS_FILE && m_strcmp(name, curr->name) == 0) {
            if (curr->data) {
                uart_print("FS_DEBUG: Eliberare memorie veche.\n");
                kfree(curr->data);
                curr->data = NULL;
                curr->size = 0;
            }
            ...
            if (size == 0 || !content) {
                uart_print("FS_DEBUG: Dimensiunea este 0, fisierul a fost golit.\n");
                return 0;
            }
            ...
            uart_print("FS_DEBUG: Scriere finalizata cu succes in RAM.\n");
            return 0;
        }
        curr = curr->next;
    }
